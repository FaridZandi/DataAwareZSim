...
public:
    AdaptiveEvent(G _get, F _fire, uint64_t _start, uint64_t _target, uint64_t _maxRate) : Event(0), get(_get),
                                                                                           fire(_fire), target(_target),
                                                                                           maxRate(_maxRate) {
...
public:
    ...
    void tick() {
        ...
        while (it != evMap.end() && it->first <= curPhase) {
            if (unlikely(it->first != curPhase)) panic("First event should have ticked on phase %ld, this is %ld",
                                                       it->first, curPhase);

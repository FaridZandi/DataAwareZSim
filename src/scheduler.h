...
private:
    ...
    enum FutexJoinAction {
        FJA_NONE, FJA_WAKE, FJA_WAIT
    };
...
public:
    Scheduler(void (*_atSyncFunc)(void), uint32_t _parallelThreads, uint32_t _numCores, uint32_t _schedQuantum) :
            atSyncFunc(_atSyncFunc), bar(_parallelThreads, this), numCores(_numCores), schedQuantum(_schedQuantum),
            rnd(0x5C73D9134) {
    ...
    void initStats(AggregateStat *parentStat) {
        ...
        threadsCreated.init("thCr", "Threads created");
        schedStats->append(&threadsCreated);
        threadsFinished.init("thFn", "Threads finished");
        schedStats->append(&threadsFinished);
        scheduleEvents.init("schedEvs", "Schedule events");
        schedStats->append(&scheduleEvents);
        waitEvents.init("waitEvs", "Wait events");
        schedStats->append(&waitEvents);
        handoffEvents.init("handoffEvs", "Handoff events");
        schedStats->append(&handoffEvents);
        sleepEvents.init("sleepEvs", "Sleep events");
        schedStats->append(&sleepEvents);
        idlePhases.init("idlePhases", "Phases with no thread active");
        schedStats->append(&idlePhases);
        idlePeriods.init("idlePeriods", "Periods with no thread active");
        schedStats->append(&idlePeriods);
        occHist.init("occHist", "Occupancy histogram", numCores + 1);
        schedStats->append(&occHist);
        uint32_t runQueueHistSize = ((numCores > 16) ? numCores : 16) + 1;
        runQueueHist.init("rqSzHist", "Run queue size histogram", runQueueHistSize);
        schedStats->append(&runQueueHist);
    ...
    void finish(uint32_t pid, uint32_t tid) {
        ...
        assert_msg(
                th->state == STARTED /*might be started but in fastFwd*/ || th->state == OUT || th->state == BLOCKED ||
                th->state == QUEUED, "gid %d finish with state %d", gid, th->state);
        if (th->state == QUEUED) {
    ...
    void leave(uint32_t pid, uint32_t tid, uint32_t cid) {
        ...
        if (th->markedForSleep) { //transition to SLEEPING, eagerly deschedule
            ...
            if (sleepQueue.empty() || sleepQueue.front()->wakeupPhase > th->wakeupPhase) {
                ...
                trace(Sched, "Put %d in sleepQueue (deadline %ld), after %d (deadline %ld)", gid, th->wakeupPhase,
                      cur->gid, cur->wakeupPhase);
    ...
    virtual void callback() {
        ...
        if (!sleepQueue.empty()) {
            ...
            while (th && th->wakeupPhase <= curPhase) {
                ...
                trace(Sched, "%d SLEEPING -> BLOCKED, waking up from timeout syscall (curPhase %ld, wakeupPhase %ld)",
                      th->gid, curPhase, th->wakeupPhase);
    ...
    uint32_t getScheduledPid(uint32_t cid) const {
        return (contexts[cid].state == USED) ? getPid(contexts[cid].curThread->gid) : (uint32_t) -1;
    }
...
private:
    ...
    void waitForContext(ThreadInfo *th) {
        ...
        futex_unlock(&schedLock);
        while (true) {
            int futex_res = syscall(SYS_futex, &th->futexWord, FUTEX_WAIT,
                                    1 /*a racing thread waking us up will change value to 0, and we won't block*/,
                                    nullptr, nullptr, 0);
    ...
    void schedTick() {
        ...
        info("Time slice ended, context-switched %d threads, runQueue size %ld, available %ld", contextSwitches,
             runQueue.size(), avail.size());
        printState();
    }
...
public:
    ...
    void syscallLeave(uint32_t pid, uint32_t tid, uint32_t cid, uint64_t pc, int syscallNumber, uint64_t arg0,
                      uint64_t arg1);

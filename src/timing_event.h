...
enum EventState {
    EV_INVALID, EV_NONE, EV_QUEUED, EV_RUNNING, EV_HELD, EV_DONE
};
...
public:
    TimingEvent(uint32_t _preDelay, uint32_t _postDelay, int32_t _domain = -1) : next(nullptr), state(EV_NONE),
                                                                                 cycle(0), minStartCycle(-1L),
                                                                                 child(nullptr),
                                                                                 domain(_domain), numChildren(0),
                                                                                 numParents(0), preDelay(_preDelay),
                                                                                 postDelay(_postDelay) {}

    explicit TimingEvent(int32_t _domain = -1) : next(nullptr), state(EV_NONE), minStartCycle(-1L), child(nullptr),
                                                 domain(_domain), numChildren(0), numParents(0), preDelay(0),
                                                 postDelay(0) {} //no delegating constructors until gcc 4.7...
    ...
    TimingEvent *addChild(TimingEvent *childEv, EventRecorder *evRec) {
        assert_msg(state == EV_NONE || state == EV_QUEUED, "adding child in invalid state %d %s -> %s", state,
                   typeid(*this).name(), typeid(*childEv).name()); //either not scheduled or not executed yet
    ...
    inline void run(uint64_t startCycle) {
        assert(this);
        assert_msg(state == EV_NONE || state == EV_QUEUED, "state %d expected %d (%s)", state, EV_QUEUED,
                   typeid(*this).name());
        state = EV_RUNNING;
        // XXX HACK

...
int launchXtermDebugger(int targetPid, LibInfo *libzsimAddrs) {
    ...
    if (childPid == 0) {
        ...
        snprintf(symbolCmdStr, sizeof(symbolCmdStr), "add-symbol-file %s %p -s .data %p -s .bss %p", QUOTED(ZSIM_PATH),
                 libzsimAddrs->textAddr, libzsimAddrs->dataAddr, libzsimAddrs->bssAddr);
        ...
        const char *const args[] = {"xterm", "-e", "gdb", "-p", targetPidStr.c_str(),
                                    ...
                                    "-ex", symbolCmdStr,
                                    "-ex",
                                    "handle SIGTRAP nostop noprint", // For some reason we receive a lot of spurious sigtraps

...
public:
    ...
    inline bool predict(Address branchPc, bool taken) {
        ...
        // Update
        pht[phtIdx] = taken ? (pred ? 3 : (pht[phtIdx] + 1)) : (pred ? (pht[phtIdx] - 1)
                                                                     : 0); //2-bit saturating counter
        bhsr[bhsrIdx] =
                ((bhsr[bhsrIdx] << 1) & histMask) | (taken ? 1 : 0); //we apply phtMask here, dependence is further away
            ...
            if (!ubWin.empty()) {
                ...
                while (it != ubWin.end() && it->first < nextWinHorizon) {
                    ...
                    assert_msg(nextWinPos < H,
                               "WindowStructure: ubWin elem exceeds limit cycle=%ld curCycle=%ld nextWinPos=%d",

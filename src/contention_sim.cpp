...
ContentionSim::ContentionSim(uint32_t _numDomains, uint32_t _numSimThreads) {
    ...
    if ((numDomains % numSimThreads) != 0) panic("numDomains(%d) must be a multiple of numSimThreads(%d) for now",
                                                 numDomains, numSimThreads);
    ...
    lastCrossing = gm_calloc<CrossingEventInfo>(
            numDomains * numDomains * MAX_THREADS); //TODO: refine... this allocs too much
}
...
void ContentionSim::enqueue(TimingEvent *ev, uint64_t cycle) {
    ...
    assert_msg(cycle >= domains[ev->domain].curCycle, "Queued event goes back in time, cycle %ld curCycle %ld", cycle,
               domains[ev->domain].curCycle);
    ev->privCycle = cycle;
...
void ContentionSim::enqueueCrossing(CrossingEvent *ev, uint64_t cycle, uint32_t srcId, uint32_t srcDomain,
    ...
    if (isResp) {
        ...
        if (last->cycle > srcDomCycle && last->cycle <=
                                         cycle) { //NOTE: With the OOO model, last->cycle > cycle is now possible, since requests are issued in instruction order -> ooo

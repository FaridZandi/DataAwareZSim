...
public:
    ...
    virtual void
    access(uint64_t accessCycle, uint64_t issuedCycle, uint32_t row, uint32_t col, uint32_t bank, MemAccessType type);
    ...
    void SetIdlePowerDownCycle(uint64_t cycle) {
        assert(idlePowerDownCycle <= cycle);
        idlePowerDownCycle = cycle;
    }

    void SetActvPowerDownCycle(uint64_t cycle) {
        assert(actvPowerDownCycle <= cycle);
        actvPowerDownCycle = cycle;
    }

    void SetIdleStandbyCycle(uint64_t cycle) {
        assert(idleStandbyCycle <= cycle);
        idleStandbyCycle = cycle;
    }

    void SaveBackgroundCycles(void);

    void SetRefreshNum(uint32_t num) {
        assert(refreshNum <= num);
        refreshNum = num;
    }
    ...
    void SetLastActCycle(uint32_t bank, uint64_t cycle) {
        assert(lastActCycle[bank] <= cycle);
        lastActCycle[bank] = cycle;
    }

    uint64_t GetLastPreCycle(uint32_t bank) { return lastPreCycle[bank]; }

    void SetLastPreCycle(uint32_t bank, uint64_t cycle) {
        assert(lastPreCycle[bank] <= cycle);
        lastPreCycle[bank] = cycle;
    }

    uint64_t GetFAWCycle(void) { return tFAWCycle[tFAWIndex]; }

    void SetFAWCycle(uint32_t bank, uint64_t cycle) {
        assert(tFAWCycle[bank] <= cycle);
        tFAWCycle[bank] = cycle;
    }

    uint64_t GetFAWCycle(uint32_t bank) { return tFAWCycle[bank]; }

    void SetFAWCycle(uint64_t cycle) {
        assert(tFAWCycle[tFAWIndex] <= cycle);
        tFAWCycle[tFAWIndex++] = cycle;
        tFAWIndex %= 4;
    }
};
...
protected:
    ...
    virtual void
    UpdatePowerDownCycle(uint32_t rank, uint64_t arrivalCycle, uint64_t lastPhaseCycle, uint32_t refreshNum);
    ...
    virtual uint64_t
    CalcIntraIssueCycle(bool rowHit, uint32_t rank, MemAccessType type, uint64_t arrivalCycle, uint32_t refreshNum);
...
public:
    ...
    virtual uint64_t
    LatencySimulate(Address lineAddr, uint64_t arrivalCycle, uint64_t lastPhaseCycle, MemAccessType type);
...
protected:
    ...
    virtual uint64_t
    CalcDQTermCur(uint64_t acc_dq, uint64_t last_dq, uint64_t instCycle, uint64_t memCycle, uint64_t lastMemCycle);
...
public:
    MemControllerBase(g_string _memCfg, uint32_t _cacheLineSize, uint32_t _sysFreqMHz, uint32_t _domain,
                      g_string &_name);
...
public:
    MemAccessEventBase(MemControllerBase *_dram, MemAccessType _type, Address _addr, int32_t domain, uint32_t preDelay,
                       uint32_t postDelay)

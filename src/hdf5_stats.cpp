...
private:
    ...
    hid_t getH5Type(
            Stat *stat) { //I'd like to make this functional, but passing a member function as an argument is non-trivial...
        ...
        if (aggrStat == nullptr) {
            ...
            for (uint32_t i = 1; i < aggrStat->size(); i++) {
                ...
                if (otherType != childType) {
                    panic("In regular aggregate %s, child %d has a different type than first child. Doesn't look regular to me!",
                          stat->name(), i);
                }
            }
...
public:
    HDF5BackendImpl(const char *_filename, AggregateStat *_rootStat, size_t _bytesPerWrite, bool _skipVectors,
                    bool _sumRegularAggregates) :
            filename(_filename), rootStat(_rootStat), skipVectors(_skipVectors),
            sumRegularAggregates(_sumRegularAggregates) {
        // Create stats file

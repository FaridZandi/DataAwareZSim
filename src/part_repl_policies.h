...
public:
    ...
    void initStats(AggregateStat *parentStat) {
        ...
        AggregateStat *partsStat = new AggregateStat(
        ...
        for (uint32_t p = 0; p < partitions; p++) {
            ...
            ProxyStat *pStat;
            pStat = new ProxyStat();
            pStat->init("sz", "Actual size", &partInfo[p].size);
            partStat->append(pStat);
            pStat = new ProxyStat();
            pStat->init("tgtSz", "Target size", &partInfo[p].targetSize);
            partStat->append(pStat);
            partInfo[p].profHits.init("hits", "Hits");
            partStat->append(&partInfo[p].profHits);
            partInfo[p].profMisses.init("misses", "Misses");
            partStat->append(&partInfo[p].profMisses);
            partInfo[p].profSelfEvictions.init("selfEvs", "Evictions caused by us");
            partStat->append(&partInfo[p].profSelfEvictions);
            partInfo[p].profExtEvictions.init("extEvs", "Evictions caused by others");
            partStat->append(&partInfo[p].profExtEvictions);
...
public:
    VantageReplPolicy(PartitionMonitor *_monitor, PartMapper *_mapper, uint64_t _lines, uint32_t _assoc,
                      uint32_t partPortionPct,
                      uint32_t partSlackPct, uint32_t maxAperturePct, uint32_t _partGranularity, bool _smoothTransients)
            : PartReplPolicy(_monitor, _mapper), totalSize(_lines), assoc(_assoc), rng(0xABCDE563F),
              smoothTransients(_smoothTransients) {
        partitions = mapper->getNumPartitions();
        ...
        info("Vantage RP: %d partitions, managed portion %f Amax %f slack %f", partitions, partPortion, maxAperture,
             partSlack);
    }
    ...
    void initStats(AggregateStat *parentStat) {
        ...
        ProxyStat *pStat;
        profPromotions.init("ffaProms", "Promotions from unmanaged region");
        rpStat->append(&profPromotions);
        profUpdateCycles.init("updCycles", "Cycles of updates experienced on size-cycle counters");
        rpStat->append(&profUpdateCycles);
        for (uint32_t p = 0; p <= partitions; p++) {
            std::stringstream pss;
            pss << "part-" << p;
            AggregateStat *partStat = new AggregateStat();
            ...
            partStat->append(pStat);
            pStat = new ProxyStat();
            pStat->init("xSz", "Extended actual size, including lines currently demoted to FFA",
                        &partInfo[p].extendedSize);
            partStat->append(pStat);
            ...
            pStat = new ProxyStat();
            pStat->init("tgtSz", "Target size", &partInfo[p].longTermTargetSize);
            partStat->append(pStat);
            pStat = new ProxyStat();
            pStat->init("stTgtSz", "Short-term target size (used with smoothedTransients)", &partInfo[p].targetSize);
            partStat->append(pStat);
            partInfo[p].profHits.init("hits", "Hits");
            partStat->append(&partInfo[p].profHits);
            partInfo[p].profMisses.init("misses", "Misses");
            ...
            partInfo[p].profDemotions.init("dems", "Demotions");
            partStat->append(&partInfo[p].profDemotions);
            partInfo[p].profEvictions.init("evs", "Evictions");
            partStat->append(&partInfo[p].profEvictions);
            partInfo[p].profSizeCycles.init("szCycles", "Cumulative per-cycle sum of sz");
            partStat->append(&partInfo[p].profSizeCycles);
            partInfo[p].profExtendedSizeCycles.init("xSzCycles", "Cumulative per-cycle sum of xSz");
            partStat->append(&partInfo[p].profExtendedSizeCycles);
    ...
    uint32_t getBestCandidate() {
        ...
        for (uint32_t i = 0; i < candIdx; i++) {
            ...
            if (((partInfo[p].curBts - e->bts) % 256) /*8-bit distance to current TS*/ >=
                ((partInfo[p].curBts - partInfo[p].setpointBts) % 256)) {
#else
            ...
            if (/*partInfo[p].curIntervalDems >= 16 || partInfo[p].curIntervalIns >= 16 ||*/
                    partInfo[p].curIntervalCands >= 256) {
...
*/
        ...
        for (uint32_t i = 0;
             i < candIdx; i++) { //note we include 0; 0 compares with itself, see shortcut to understand why
        ...
        for (uint32_t p = 0; p < partitions; p++) {
            ...
            if (smoothTransients) {
                ...
                if (s[p] > partInfo[p].targetSize) { //growing
                    uint32_t newTarget = MAX(partInfo[p].targetSize, MIN(partInfo[p].longTermTargetSize,
                                                                         partInfo[p].size +
                                                                         1)); //always in [target,longTermTarget]

...
private:
    ...
    public:
        ...
        template<typename C>
        uint32_t rank(const MemReq *req, C cands) {panic("!!!"); }
...
public:
    IdealLRUPartReplPolicy(PartitionMonitor *_monitor, PartMapper *_mapper, uint32_t _numLines, uint32_t _numBuckets)
            : PartReplPolicy(_monitor, _mapper), numLines(_numLines), numBuckets(_numBuckets) {
    ...
    void initStats(AggregateStat *parentStat) {
        ...
        for (uint32_t p = 0; p < partitions; p++) {
            ...
            partInfo[p].profExtEvictions.init("extEvs", "Evictions caused by others (in transients)");
    ...
    uint32_t rank(const MemReq *req) {
        ...
        for (uint32_t p = 0; p < partitions; p++) {
            double diff = ((int32_t) partInfo[p].size - (int32_t) partInfo[p].targetSize) /
                          ((double) (partInfo[p].targetSize + 1));
            //info("YYY %d %f", p, diff);
        ...
        if (maxPartDiff < -1e-8) {
            warn("Evicting from non-full part! diff=%f victimPart %d (sz %ld tgt %ld) origPart %d",
                 maxPartDiff, victimPart, partInfo[victimPart].size, partInfo[victimPart].targetSize,
                 mapper->getPartition(*req));
        }
